...
class QCTab(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.crm_db_path = parent.resource_path("crm_blank.db")
        self.ver_db_path = parent.resource_path("crm_data.db")
        ...
        # Apply style
        for widget in [self.device_combo, self.crm_combo, self.element_combo,
                    self.from_date_edit, self.to_date_edit, self.percentage_edit]:
            widget.setStyleSheet(input_style)

        # Set minimum widths
        self.device_combo.setMinimumWidth(180)
        self.crm_combo.setMinimumWidth(160)
        self.element_combo.setMinimumWidth(160)
        self.from_date_edit.setFixedWidth(140)
        self.to_date_edit.setFixedWidth(140)

        # Add label + widget pairs
        filter_items = [
            ("Device:", self.device_combo),
            ("CRM ID:", self.crm_combo),
            ("Element:", self.element_combo),
    ...
    def load_default_logo(self):
        if self.logo_path.exists():
            ...
            self.logo_label.setPixmap(pixmap.scaled(130, 130, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def update_element_combo(self, target_element=None):
        """ایجاد دکمه‌های عناصر بر اساس Device و CRM"""
        if self.updating_filters:
            return
        ...
        try:
            ...
            if not filtered.empty:
                ...
                # استایل دکمه‌های خیلی کوچک + رنگ اصلی از اول
                button_style = """
                    QPushButton {
                        padding: 2px 2px;
                ...
                # ایجاد دکمه‌ها (حداکثر 12 تا - چون کوچیکن جا میشن)
                ...
                for i, element in enumerate(elements):
                    ...
                    self.element_buttons_layout.addWidget(btn)
                    
                    # اتصال سیگنال
                    btn.toggled.connect(self.on_element_changed)
                    
                    # انتخاب خودکار اولین دکمه یا target_element
                    if (not self.selected_element and i == 0) or (target_element == element):
                        btn.setChecked(True)
                        self.selected_element = element
                        first_selected = True
                
                # اگر هیچ دکمه‌ای انتخاب نشده، اولین رو انتخاب کن
            ...
            logger.info(f"Element buttons created: {len(self.element_buttons)} elements")
